...
if (
  localStorage.getItem("pipeJSON") === null ||
  localStorage.getItem("pipeJSON") === undefined
) {
...
class DashboardPage extends React.Component {
  render() {
    ...
    API.getUserAndWells({ uid: uid }).then(response => {
      ...
      sessionStorage.setItem(
        "wellData",
        JSON.stringify(response.data.wellData)
      );
    });
    return (
      <div>
        <hr />
        <ul>
          <Link className="link-class" to="/new-well">
            <button>new well</button>
          </Link>{" "}
          <br /> <br />
          <Link className="link-class" to="/my-wells">
            <button>my wells</button>
          </Link>{" "}
          <br /> <br />
          <Link className="link-class" to="/account">
